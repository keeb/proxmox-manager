...
import { grafanaApiGet, grafanaApiPost, grafanaApiPut, grafanaApiPostFile } from "./lib/grafana.ts";
...
const QueryMetricsArgs = z.object({
  ...
  methods: {
    discover: {
      ...
      execute: async (_args, context) => {
        ...
        const handle = await context.writeResource("instance", "instance", {
          ...
          dashboards: JSON.stringify(dashboards.map(d => ({ uid: d.uid, title: d.title }))),
          datasources: JSON.stringify(datasources.map(ds => ({ name: ds.name, type: ds.type }))),
    ...
    pushDashboard: {
      ...
      execute: async (args, context) => {
        ...
        // Ensure id is null so Grafana resolves by UID (create-or-update)
        dashboardJson.id = null;

        // Write wrapped payload to a temp file
        const payload = JSON.stringify({ dashboard: dashboardJson, overwrite: true });
    ...
    pushAlertRule: {
      ...
      execute: async (args, context) => {
        ...
        // Use file-based upload to avoid shell escaping $labels in annotation templates
        ...
        // @ts-ignore - Deno API
        try { await Deno.remove(tmpFile); } catch (_e) { /* ignore */ }
    ...
    queryMetrics: {
      ...
      execute: async (args, context) => {
        ...
        // Discover Prometheus datasource ID
        ...
        // Parse start time — relative (e.g. "24h", "7d") or absolute
        ...
        if (relMatch) {
          ...
          startSec = Math.floor(new Date(start).getTime() / 1000);
        }
        ...
        if (end) {
          ...
          if (relEnd) {
            ...
            endSec = Math.floor(new Date(end).getTime() / 1000);
          }
        } else {
          endSec = nowSec;
        }
        ...
        console.log(`[queryMetrics] Range: ${new Date(startSec * 1000).toISOString()} → ${new Date(endSec * 1000).toISOString()} step=${step}`);

        // Query via datasource proxy — use GET with URL-encoded params to avoid shell escaping PromQL
        ...
        for (const s of series) {
          ...
          if (nonZero.length > 0 && nonZero.length <= 50) {
            for (const v of nonZero) {
              const ts = new Date(v[0] * 1000).toISOString();
              console.log(`    ${ts}  ${v[1]}`);
            }
